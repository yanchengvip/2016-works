<template>
  <!-- 普通用户任务 -->
  <div class="task" v-show="data.type==0 || data.type==1 || data.type==2 || data.type==3">
    <div class="content user" :class="{ grey: data.is_start == 2 && data.current_step == 0 , ok: isOk, 'not-ok': notOk }">
      <div class="region-limit" v-show="data.belong_city">仅限{{ data.belong_city }}使用</div>
      <div class="title">{{ data.title }}</div>
      <ul v-if="data.type == 0" class="commission">
        <li >¥</li>
        <li>0</li>
        <li v-for="item in data.current_amount" :style="{ color: '#17a2fd'  }" track-by="$index">{{ item }}</li>
      </ul>
      <div v-if="data.type != 0" class="progress" :style="{backgroundSize: progressPercent+'% 100%'}">{{ progressText }}</div>
      <div class="line"></div>
      <table class="message">
        <tbody>
          <tr>
            ...
            <td><img src="../../assets/task/3.svg"></td>
          </tr>
          <tr>
            <td v-if="data.is_start == 2 || data.is_start == 4 || data.is_start == 5">已结束</td>
            <td v-else><span class="main-color">{{data.end_days}}</span>后结束</td>
            <td>订单<span class="order-num">＋{{ data.orders_num }}</span></td>
            <td v-if="data.type == 0"><span class="reward-num">¥{{ data.current_reward }}</span></td>
            <td v-if="data.type != 0"><span class="reward-num">¥{{rewardProgress[0]}}</span>/¥{{rewardProgress[1]}}</td>
          </tr>
        </tbody>
      </table>
    </div>
...
.progress {
    ...
    border-radius: 1rem;
    background-image: url('../../assets/task/bar.png');
    background-repeat: no-repeat;
    margin-top: .8rem;
}
.progress-promoter {
